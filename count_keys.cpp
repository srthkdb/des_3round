...
int main(){
    ...
    FILE *f_inp, *f_inp_xor, *f_key, *fd, *f_out_xor;
    f_inp_xor = fopen("sbox_in_xor.txt","r+");
    f_key = fopen("key.txt","w+");
    f_out_xor = fopen("sbox_out_xor", "r+");
        f_inp = fopen("sbox_in_before_key.txt","r+");
    ...
    for(int text = 0 ; text < number_of_text; text++){
        ...
        fscanf(f_inp,"%s",sef);
        fscanf(f_inp,"%s",sef_star);
        fscanf(f_inp_xor,"%s",sif_dash);
        fscanf(f_out_xor,"%s",sof_dash);
